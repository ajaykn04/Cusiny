...
app.get("/recipe/get/:id", async (req, res) => {
  try {
    ...
    const reviews = await runQuery("SELECT * FROM review WHERE recipe_id = ?", [
      id,
    ]);
...
app.post("/user/add", async (req, res) => {
  try {
    await runQuery(
      "INSERT INTO user (username, email, place, age, password, admin) VALUES (?, ?, ?, ?, ?, ?)",
      [
        req.body.username,
        req.body.email,
        req.body.place,
...
app.get("/user/get/:email/:password", async (req, res) => {
  try {
    ...
    const password = CryptoJS.SHA256(req.params.password).toString(
      CryptoJS.enc.Hex
    );
...
app.post("/user/register/", async (req, res) => {
  try {
    ...
    const existing_user = await runQuery("SELECT * FROM user WHERE email = ?", [
      user.email,
    ]);
...
app.put("/user/edit/", async (req, res) => {
  try {
    ...
    if (existing_user.length === 0) {
      await runQuery(
        "UPDATE user SET username = ?, place = ?, age = ? WHERE _id = ?",
        [user.username, user.place, user.age, id]
      );
      res.send({ message: "Profile Updated" });
    } else if (existing_user[0]._id === user._id) {
      await runQuery(
        "UPDATE user SET username = ?, email = ?, place = ?, age = ? WHERE _id = ?",
...
app.put("/recipe/edit/", upload.single("file"), async (req, res) => {
  try {
    ...
    const db_recipe = await runQuery("SELECT * FROM recipe WHERE _id = ?", [
      id,
    ]);
    const recipeData = db_recipe[0];
    await runQuery(
      "UPDATE recipe SET name = ?, ingredients = ?, instructions = ?, category = ? WHERE _id = ?",
      [
        recipe.name,
        recipe.ingredients,
        recipe.instructions,
    ...
    if (req.file) {
      const img_path = `images/recipes/${recipe._id}${path.extname(
        req.file.filename
      )}`;
      fs.unlink(recipeData.image, () => {});
      fs.rename(req.file.path, img_path, () => {});
      await runQuery("UPDATE recipe SET image = ? WHERE _id = ?", [
        img_path,
        id,
      ]);
    }
...
app.post("/recipe/add/", upload.single("file"), async (req, res) => {
  try {
    ...
    const result = await runQuery(
      "INSERT INTO recipe (owner, ownername, name, ingredients, instructions, category, image, rating) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [
        recipe.owner,
        recipe.ownername,
        recipe.name,
    ...
    const img_path = `images/recipes/${result.insertId}${path.extname(
      req.file.filename
    )}`;
    fs.rename(req.file.path, img_path, () => {});
    await runQuery("UPDATE recipe SET image = ? WHERE _id = ?", [
...
app.post("/recipe/addreview/:recipeId", async (req, res) => {
  try {
    ...
    await runQuery("insert into review values (0, ?, ?, ?, ?, ?)", [
      id,
      review.user_id,
      review.username,
      review.rating,
    ...
    const allReviews = await runQuery(
      "SELECT rating FROM review WHERE recipe_id = ?",
      [id]
    );
    ...
    await runQuery("UPDATE recipe SET rating = ? WHERE _id = ?", [
      averageRating,
      id,
    ]);
...
app.delete("/recipe/delreview/:recipeId/:userId", async (req, res) => {
  try {
    ...
    const recipe = await runQuery("SELECT * FROM recipe WHERE _id = ?", [
      recid,
    ]);
    ...
    await runQuery("UPDATE recipe SET reviews = ?, rating = ? WHERE _id = ?", [
